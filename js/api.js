...
function abrirModalEditarProducto(id) {
    ...
    let p = null;
    if (window.inventarioGlobalCache && window.inventarioGlobalCache[id]) {
        p = window.inventarioGlobalCache[id];
    } else if (typeof inventarioGlobalCache !== 'undefined' && inventarioGlobalCache && inventarioGlobalCache[id]) {
        p = inventarioGlobalCache[id];
    } else if (typeof listaGlobalProductos !== 'undefined' && Array.isArray(listaGlobalProductos)) {
        ...
        fetch(`${API_URL}?accion=obtenerInventario&nocache=${Date.now()}`)
            ...
            .then(data => {
                ...
                if (data && data[id]) {
                    ...
                    alert(`No se encontró el producto "${id}" en el inventario.`);
                }
            })
            .catch(err => alert("Error al consultar el inventario: " + err.message));
        return;
    }
